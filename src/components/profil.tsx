...
export default function Profil() {
  ...
  const [changeFirstName, { loading: changingFirstName }] = useMutation(
    CHANGE_FIRST_NAME_MUTATION,
  );
  const [changeLastName, { loading: changingLastName }] = useMutation(
    CHANGE_LAST_NAME_MUTATION,
  );
  ...
  return (
    <>
      <div className="min-h-screen grid grid-cols-1 xl:grid-cols-4 bg-base-200 mx-auto gap-2 p-4">
        <div className="card xl:w-96 bg-base-100 shadow-xl p-6">
          ...
          {error && (
            <p className="text-error">
              {t("common.error")}:<br /> {error.message}
            </p>
          )}
          {data != undefined && (
            <div>
              ...
              <p>
                <strong>{t("profil.firstName")}:</strong>{" "}
              ...
              <p>
                <strong>{t("profil.lastName")}:</strong>{" "}
              ...
              {dataTimbre &&
                ...
                dataTimbre.myTimbres.map((timbre) => (
                  ...
                  >
                    ...
                    <a
                      href={`http://127.0.0.1:8000/${timbre.pdfFile.url}`}
                      target="_blank"
          ...
          <div className="py-4 flex flex-col gap-4">
            <form
              ...
              onSubmit={(e) => {
                ...
                changeFirstName({ variables: { nom: firstName } })
                  .then((res) => {
                    if (res.data?.changeFirstName.success) {
                      toast.success(res.data.changeFirstName.message, {
                        id: toastId,
                      });
                      refetch();
                    } else {
            ...
            >
              <div className="form-control w-full flex items- justify-between gap-4">
                ...
                <button
                  type="submit"
                  className="btn btn-outline btn-info btn-ghost"
                >
                  {changingFirstName ? (
            ...
            <form
              ...
              onSubmit={(e) => {
                ...
                const toastId = toast.loading(`${t("common.pleaseWait")}`);
                changeLastName({ variables: { nom: lastName } })
                  .then((res) => {
                    if (res.data?.changeLastName.success) {
                      toast.success(res.data.changeLastName.message, {
                        id: toastId,
                      });
                      refetch();
                    } else {
          ...
          <div className="py-4">
            <form
              ...
              onSubmit={(e) => {
                ...
                const formData = new FormData(e.currentTarget);
                const oldPassword =
                  formData.get("old_password")?.toString() ?? "";
                const newPassword1 =
                  formData.get("new_password1")?.toString() ?? "";
                const newPassword2 =
                  formData.get("new_password2")?.toString() ?? "";
                if (
                  newPassword1 === "" ||
                  newPassword2 === "" ||
                  oldPassword === ""
                ...
                changePassword({
                  ...
                  .then((res) => {
                    if (res.data?.changePassword.success) {
                      toast.success(t("profil.passwordChangedSuccessfully"), {
                        id: toastId,
                      });
                    } else {
                      toast.error(
                        res.data?.changePassword.errors.join("\n") ??
                          t("common.error"),
                        { id: toastId },
                      );
                    }
            ...
            >
              ...
              <button
                type="submit"
                className={`btn ${changingPassword ? "loading" : ""}`}
              >
                {t("profil.ChangePassword")}
              </button>
            </form>

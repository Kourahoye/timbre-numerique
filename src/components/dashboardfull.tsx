...
interface Transaction {
  ...
  status: string;
  createdAt: string;
  timbre: { id: string; type: { name: string } };
}
interface QueryData {
  ...
  return (
    <div className="card bg-base-200 border border-base-300 shadow-sm">
      <div className="card-body p-4 gap-1">
        ...
        <span
          className={`font-mono text-2xl font-bold ${color ?? "text-base-content"}`}
        >
          {value}
        </span>
...
function UsageDonut({
  ...
  const data = [
    { name: trans("timbre.used_s"), value: used },
    { name: trans("timbre.available_s"), value: unused },
  ];
  return (
    <ChartCard title={trans("dashboard.globalUse")}>
      ...
      <div className="flex justify-around">
        {[
          { label: trans("timbre.used_s"), val: used, color: pal.p },
          { label: trans("timbre.available_s"), val: unused, color: pal.dim },
        ].map((d) => (
...
function RevenueBar({
  ...
  const data = sessionInfos.map((s) => {
    ...
    return {
      name: s.name,
      [trans("timbre.revenus")]: Math.round(
        st.filter((t) => t.used).reduce((sum, t) => sum + t.price.price, 0),
      ),
    };
  });
  return (
    <ChartCard title={trans("dashboard.revenueBySession")}>
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
          ...
          <XAxis
            dataKey="name"
            tick={{ fontSize: 11, fill: pal.dim }}
            axisLine={false}
            tickLine={false}
          />
          <YAxis
            tick={{ fontSize: 10, fill: pal.dim }}
            axisLine={false}
            tickLine={false}
            width={44}
            tickFormatter={fmtK}
...
function TimbreStackBar({
  ...
  const data = sessionInfos.map((s) => {
    ...
    return {
      name: s.name,
      [`${trans("timbre.used_s")}`]: used,
      [`${trans("timbre.available_s")}`]: st.length - used,
    };
  });
  return (
    <ChartCard title={trans("dashboard.bySessionStamp")}>
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
          ...
          <Bar
            dataKey={trans("timbre.available_s")}
            stackId="a"
            fill={pal.s}
            radius={[4, 4, 0, 0]}
          />
...
function TypeBar({
  ...
  const data = timbreType.map((tt) => {
    ...
    return {
      name: tt.name,
      [`${trans("timbre.total")}`]: items.length,
      [`${trans("timbre.used_s")}`]: items.filter((t) => t.used).length,
    };
  });
  return (
    <ChartCard title={trans("dashboard.stampByType")}>
      <ResponsiveContainer width="100%" height={200}>
        ...
        >
          ...
          <Legend iconSize={8} wrapperStyle={{ fontSize: 11 }} />
          <Bar
            dataKey={trans("timbre.total")}
            fill={pal.a}
            radius={[0, 4, 4, 0]}
          />
          <Bar
            dataKey={trans("timbre.used_s")}
            fill={pal.p}
            radius={[0, 4, 4, 0]}
          />
        </BarChart>
...
function TxTrend({
  ...
  const data = days.map((day) => {
    ...
    return {
      date: day.slice(5),
      [trans("transaction.status.accepted")]: tx.filter(
        (t) => t.status === "accepted",
      ).length,
      [trans("transaction.status.pending")]: tx.filter(
        (t) => t.status === "pending",
      ).length,
      [trans("transaction.status.rejected")]: tx.filter(
        (t) => t.status === "rejected",
      ).length,
    };
  });
    ...
    <ChartCard title={trans("dashboard.evolution")}>
      <ResponsiveContainer width="100%" height={220}>
        ...
        >
          ...
          <Line
            type="monotone"
            dataKey={trans("transaction.status.accepted")}
            stroke={pal.ok}
            strokeWidth={2}
            dot={false}
          />
          <Line
            type="monotone"
            dataKey={trans("transaction.status.pending")}
            stroke={pal.warn}
            strokeWidth={2}
            dot={false}
            strokeDasharray="4 2"
          ...
          <Line
            type="monotone"
            dataKey={trans("transaction.status.rejected")}
            stroke={pal.err}
            strokeWidth={2}
            dot={false}
          />
        ...
        <table className="table table-sm">
          ...
          <tbody>
            {sessionInfos.map((s) => {
              ...
              const revenue = st
                .filter((t) => t.used)
                .reduce((sum, t) => sum + t.price.price, 0);
              return (
                <tr
                  key={s.id}
                  className="border-base-300 hover:bg-base-300/30 transition-colors"
                >
                  <td className="font-mono font-semibold text-sm text-base-content">
                    {s.name}
                  </td>
                  ...
                  <td className="text-right font-mono text-sm text-base-content">
                    ...
                    <span className="text-base-content/40 text-[10px] ml-1">
                      GNF
                    </span>
                  </td>
                  <td>
...
export default function DashboardFull() {
  ...
  const [theme, setTheme] = useState<ThemeName>(
    `${(localStorage.getItem("dash-theme")?.toString() as ThemeName) || "Timbre"}`,
  );
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        {/* Header */}
        <div className="flex flex-wrap items-start justify-between gap-3 mb-2 glass p-2">
          <div className="">
            <h1 className="text-xl font-bold tracking-tight text-base-content">
              {trans("dashboard.name")}
            </h1>
            <p className="text-[10px] font-bold uppercase tracking-widest text-base-content mt-0.5">
            ...
            {s?.activeSession && (
              <div className="badge badge-outline badge-lg gap-2">
                ...
                <span className="text-xs">
                  {trans("session.title")} : {s.activeSession.name}
                </span>
              </div>
            )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {loading && !s ? (
            ...
            <>
              <KpiCard
                label={trans("timbre.total")}
                value={fmt(s.totalTimbres)}
              />
              <KpiCard
                label={trans("timbre.used_s")}
                value={fmt(s.usedTimbres)}
                color="text-primary"
              />
              <KpiCard
                label={trans("timbre.available_s")}
                value={fmt(s.unusedTimbres)}
                color="text-secondary"
              />
              <KpiCard
                label={trans("timbre.revenus")}
                value={`${fmtK(s.totalRevenue)} GNF`}
                sub={trans("timbre.used_s")}
                color="text-accent"
              />
              <KpiCard label={trans("auth.users")} value={fmt(s.totalUsers)} />
              <KpiCard
                label={trans("notifications.notifications")}
                value={s.unreadNotifications}
                sub={trans("notifications.unRead")}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {loading && !s ? (
            ...
            <>
              <RevenueBar
                sessionInfos={data.sessionInfos}
                timbres={data.timbres}
                pal={pal}
              />
              <TimbreStackBar
        ...
        {loading && !s ? (
          ...
          <TxTrend transactions={data.transactions} pal={pal} />
        ) : null}

...
export default function Transaction() {
  ...
  const [scan, { loading, error, data }] = useLazyQuery<ScanData, ScanAgrs>(
    SCAN,
    {
  ...
  const consume = () => {
    ...
    if (data) {
      consumeTimbre({
        variables: { id: Number.parseInt(data?.scan.id.toString()) },
      })
        .then((res) => {
          if (res.data) {
            toast.success(`${t("transaction.requestSuccess")}`, {
              id: toastId,
            });
          }
          if (res.error) {
  ...
  return (
    <>
      <div className="min-h-screen flex flex-col justify-center items-center max-w-6xl mx-auto">
        <form
          onSubmit={(e) => {
            scanCode(e);
          }}
        ...
        >
          <div className="flex items-center justify-center space-x-2 mb-5">
            <div className="form-control input input-bordered  w-full max-w-xs">
              <label className="label">
                <span className="label-text dark:text-white font-mono">
                  {t("scan.code")}
                </span>
              </label>
              <input
          ...
          {data != undefined && (
            <div className="card w-96 bg-base-100 shadow-xl p-6">
              ...
              <p>
                <strong>{t("timbre.reference")}:</strong> {data.scan.reference}
              </p>
              <p>
                <strong>{t("timbre.type")}:</strong> {data.scan.type.name}
              </p>

...
export default function Session() {
  ...
  const [changeName, { loading: changingName }] =
    useMutation(CHANGE_SESSION_NAME);
  const [changeDates, { loading: changingDates }] =
    useMutation(CHANGE_SESSION_DATE);
        ...
        <dialog id="update_session" className="modal">
          <div className="modal-box glass">
            ...
            <div className="py-4">
              <form
                method="post"
                className="space-x-2 space-y-2 flex justify-between"
                onSubmit={(e) => {
                  e.preventDefault();
                  if (!currId) {
              ...
              <form
                ...
                onSubmit={(e) => {
                  ...
                  const toastId = toast.loading(`${t("common.pleaseWait")}`);
                  changeDates({
                    variables: { id: currId, start: start, end: end },
                  })
                    .then((res) => {
                      if (res.data) {
                        toast.success(`${t("session.changeSuccess")}`, {
                          id: toastId,
                        });
                        refetch();
                        return;
              ...
              >
                <div className="form-control w-full ">
                  ...
                  <p className="validator-hint">
                    <span>{t("common.fieldRequired")}</span>
                  </p>
                </div>
                <div className="form-control w-full ">
                  <label className="label">
                ...
                addsession({
                  ...
                  .then((res) => {
                    ...
                    if (data.addSession.id) {
                      toast.success(
                        `${t("session.addSuccess", { name: data.addSession.name })}`,
                        {
                          id: toastId,
                        },
                      );

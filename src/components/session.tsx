...
import type { AddSession, DeleteSessionMutationResponse, Sessions, ToggleSessionMutationResponse } from "./types";
...
export default function Session (){
  ...
  const toogle = (id:number,active:boolean) =>{
    ...
    toogleSession({variables: { id: id }}).then((res) => {
        const data: ToggleSessionMutationResponse = res.data as ToggleSessionMutationResponse;
  ...
  const sessionDelete = (id:number) => {
    ...
    const toastId = toast.loading("Please wait...")
    deleteSession({variables: { id: id }}).then((res) => {
      // toast.success("Type de timbre supprimé avec succès !");
      // console.log(data)
        const data: DeleteSessionMutationResponse = res.data as DeleteSessionMutationResponse;
     ...
     <div className="min-h-screen flex flex-col gap-4 justify-center items-center bg-base-200 w-full">
        <dialog id="add_modal" className="modal">
          <div className="modal-box">
            ...
            <div className="p-6">
            <h1 className="text-2xl font-bold text-center mb-4">Créer une nouvelle session </h1>
            <form method="post" className="space-y-3 flex flex-col items-center" onSubmit={(e)=>{
              ...
              addsession({variables:{name:name,start:start,end:end}}).then((res)=>{
                const data = res.data as AddSession
                // console.log(data)
                // if (data == null){
                // }
                if (data.addSession.id){
                  toast.success(`Session ${data.addSession.name} added`,{id:toastId})
                }
                refetch();
              }).catch((err)=>{
                if(err.message.includes("UNIQUE constraint failed: timbre_session.name")){
                  toast.error("Une session avec ce nom existe deja",{id:toastId})
                  return
                }
                ...
                <div className="form-control w-full">
                  ...
                  <input type="date" placeholder="Type here"  required name="date_start" className="input input-bordered w-full" />
                  {/* <label className="label">
                    <span className="label-text-alt"></span>
                    <span className="label-text-alt"></span>
                  </label> */}
                ...
                <div className="form-control w-full">
                  ...
                  <input type="date" placeholder="Type here" required name="date_end" className="input input-bordered w-full" />
                  {/* <label className="label">
                    <span className="label-text-alt"></span>
                    <span className="label-text-alt"></span>
                  </label> */}
                ...
                <button className='btn btn-sm btn-outline btn-info btn-ghost'><span>Enregistrer</span>
                  {
        ...
        <div className="card bg-base-100 shadow-xl p-6 ml-4">
            <h1 className="text-2xl font-bold text-center mb-4 flex justify-between"><span>Sessions</span>
              <button className="btn btn-xs btn-info btn-outline btn-ghost" onClick={() => {(document.getElementById("add_modal") as HTMLDialogElement | null)?.showModal();}}>
              <RiAddLine size={20} /></button>
        </h1>
              <div className="overflow-x-auto">
                <table className="table">
                  ...
                  <tbody>
                    {
                       loading && <tr className="text-center"><td colSpan={7}><span className="loading loading-lg loading-spinner"></span> </td></tr>
                   }
                    {
                       called && data && data?.sessionInfos.length === 0 && <tr><td className="text-xl" colSpan={7}>Aucune session trouvée</td></tr>
                    }
                     {
                                           data && data.sessionInfos.length > 0 && (
                                            data.sessionInfos.map((session) => (
                                                ...
                                                <td className="text-center">
                                                  <input type="checkbox" name="toggler" onClick={(e)=>{
                                                    e.preventDefault()
                                                    toogle(Number.parseInt(session.id),session.active,)
                                                  }} checked={session.active} className={`toggle ${session.active ? "text-green-600" : "text-red-600"} `}/>
                                                </td>
                                                ...
                                                <td>
                                                  ...
                                                  <td>
                                                    <button className="btn btn-sm btn-error btn-outline btn-ghost" onClick={()=>{sessionDelete(Number.parseInt(session.id))}} >

                                                    <RiDeleteBin6Line size={20} className="text-error cursor-pointer" />
                                                    </button>
